...
def make_cmems_download(cmems_options, make_download, output_directory, ods):
    ...
    while work_date <= end_date:
        ...
        if args.verbose or not make_download:
            print(f'[INFO]{work_date.strftime("%Y-%m-%d")}:{bucket}/{key}->{available}')
        if make_download and available:
            folder_out = get_folder_out(work_date, output_directory, ods)
...
def get_folder_out(work_date, od, ods):
    if ods is None:
        return od
    ods_parts = ods.split('/')
    for part in ods_parts:
        ...
        if not create_if_not_exists(od):
            return None
    return od
