...
def add_chl_from_cci_1km():
    ...
    for idx in range(insitu_time.shape[0]):
        ...
        if os.path.exists(file_day):
            dday = Dataset(file_day)
            chla_array  = dday.variables['chlor_a'][:]
            if lat_array is None and lon_array is None:
                lat_array = dday.variables['lat'][:]
                lon_array = dday.variables['lon'][:]
            dday.close()
    ...
    var = dset.createVariable('mu_satellite_CHL_CCI','f4',('mu_id',),zlib=True,complevel=6,fill_value=-999)
    var.comment = 'Chl-a from CCI 1km: Chlorophyll-a concentration in seawater (not log-transformed), generated by as a blended combination of OCI, OCI2, OC2 and OCx algorithms, depending on water class memberships'
    var[:] = chla_cci_1km[:]
    dset.close()
