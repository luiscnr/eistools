...
def do_download(filename,odir,appkey):


    filelist = [filename]

    outpath = Path.resolve(Path.expanduser(Path(os.path.expandvars(odir))))


    failed = None

    for request in filelist:
        status = retrieveURL(request, localpath=outpath, uncompress=False,verbose=True, force_download=False,appkey=appkey, checksum=False)
        if status:
            if status == 304:
                if args.verbose:
